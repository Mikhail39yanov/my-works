...
import { consoleAxiosError, makeSessionFile, readSession } from '../utils/index.js'

export const performLogin = async (url: string) => {
  try {
    const dataPrompt = await inquirer.prompt<IUserPrompt>(questions)
    ...
    const session = await readSession(sessionFileName)

    if (session) {
      return console.log(chalk.yellow('Сессия активирована, сначала выполните команду npm run logout'))
    }
